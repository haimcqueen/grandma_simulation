...
Fetched and merged remote `main` at `4cd0d53`, preserving the teammate's fall overlay alongside the house controls. The ottoman regression passed in an isolated browser run; a preceding parallel run returned an empty GPU pixel sample at its floor-color assertion. No collision assertion failed. Changes remain local for review before pushing.
