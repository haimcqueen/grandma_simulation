...
const builders: Record<string, Builder> = {
  ...
  stove_knobs_reachable: (scene, zone, tk) => {
    for (let i = -1; i <= 1; i++) {
      ...
      knob.position.set(zone.x, 0.75, zone.z + i * 0.14);
      scene.add(knob);
    }
  },
  pot_handles_outward: (scene, zone, tk) => {
    ...
    pot.position.set(zone.x + 0.25, 1.02, zone.z + 0.1);
    pot.castShadow = true;
    scene.add(pot);
    for (const dz of [-0.16, 0.16]) tk.box(scene, zone.x + 0.03, 1.02, zone.z + 0.1 + dz, 0.18, 0.02, 0.03, 0x2e2e2c);
  },
