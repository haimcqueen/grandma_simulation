...
export class HazardTracker {
  ...
  update(
    ...
    // A profile change must re-evaluate the same physical zone's severity/reason.
    const key = hit ? `${hit.condition}:${zoneKey(hit.zone)}` : null;
    if (key !== state.key) {
      const previousPending = state.pending;
      state.key = key;
      state.pending = hit;
      if (previousPending) this.onExit?.(entityId, previousPending);
      if (hit) {
