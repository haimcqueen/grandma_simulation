...
<section class="control-section"><div class="section-number">03 <span>Fall animations</span></div><label class="field-label" for="fall-kind">Situation</label><select id="fall-kind">${roomFalls.map(fall => `<option value="${fall.id}">${fall.label}</option>`).join("")}</select><p class="source-note" id="fall-description"></p><button id="play-fall" class="routine-button">▶ Play fall</button><p class="source-note">Manual demos: replay or reset. Hazard falls: get up and continue automatically.</p><p class="source-note">Fall demos use the active floor. Stair traversal is authored; falls are disabled during a floor transfer.</p></section>
...
function renderUI() {
  ...
  element("map-hazard-legend").hidden = !viewer.hazardPropsVisible || !simulation.environment.hazardZones?.length;
